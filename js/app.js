...
function filterRows() {
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (
      //Hide all rows that do not include the searchTermь
      searchTerm.length > 0 &&
...
function highlightToday() {
  for (var i = 0; i < rows.length; i++) {
    ...
    if (html.includes(`<td>${getDay()}</td>`)) {
      ...
      rows[i].className =
        "highlighted" + (className.length == 0 ? "" : ` ${className}`); //Add class 'highlighted'
    }
  }
}
...
function determineTheme() {
  ...
  return hours < darkThemePeriod.min && hours > darkThemePeriod.max
    ? "light"
    : "dark";
}
